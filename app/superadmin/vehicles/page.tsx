...
export default async function SuperadminVehiclesPage({
  searchParams,
}: {
  ...
  const vehicles = await prisma.vehicle.findMany({
    where: q
      ? {
          OR: [
            { spz: { contains: q, mode: "insensitive" } },
    ...
    <main className="relative min-h-screen px-6 py-8">
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        ...
        <h1 className="text-2xl font-extrabold mb-6">
          Vozidla <span className="text-muted font-normal">({vehicles.length})</span>
        </h1>
        ...
        <form className="mb-6">
          <input
            ...
            name="q"
            defaultValue={q}
            placeholder="Hledat podle SPZ, značky, modelu nebo firmy…"
        ...
        <div className="glass-panel overflow-hidden">
          {vehicles.length === 0 ? (
            <div className="p-6 text-center text-sm text-muted">Žádné vozidlo neodpovídá hledání.</div>
          ) : (
            vehicles.map((v, i) => {
              const statuses = [
